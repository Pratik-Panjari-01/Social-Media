...
const StoryModel = ({setShowModel, fetchStories}) => {
    ...
    const [previewUrl,setPreviewUrl] = useState("text");
    ...
    const handleCreateStory = async()=>
    {
      ...
      fromData.append('media_type',media_type);
      fromData.append('media',media);
      fromData.append('background_color',background);

      const token = await getToken()

      try
      {
        const {data} = await api.post('/api/story/create', fromData,{
    ...
    text-white flex items-center justify-center p-4'>
      <div className='w-full max-w-md'>
        ...
        <div className='flex mt-4 gap-2'>
          {bgColors.map((color)=>(<button key={color} className='w-6 h-6
          rounded-full ring cursor-pointer' style={{backgroundColor : color}} onClick={()=>
            setBackground(color)}/>
            ))}
        </div>
