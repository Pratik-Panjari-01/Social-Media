...
import { dummyConnectionsData as connections,
  ...
  dummyPendingConnectionsData as pendingConnections} from '../assets/assets.js'
          ...
          {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
            ...
            shadow rounded-md'>
              ...
              <div className='flex-1'>
                ...
                <p className='text-sm text-slate-700'>{user.bio.slice(0,30)}...</p>
                  ...
                  }
                  {
                    currentTab === 'Connections' && (
                      <button onClick={()=>navigate(`/messages/${user._id}`)} className='w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200
                       text-slate-800 active:scale-95 transition cursor-pointer flex items-center justify-center
                       gap-1'>
